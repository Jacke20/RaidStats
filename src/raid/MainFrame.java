...
public class MainFrame {
	...
	charWindow.setBorder(BorderFactory.createTitledBorder(null,
		"Character Information", TitledBorder.TOP, TitledBorder.TOP,
		new Font("times new roman", Font.PLAIN, 12), Color.BLACK));
	charWindow.setPreferredSize(new Dimension(320, 200));
	groupWindow.setLayout(new BorderLayout());
	groupWindow.setBorder(BorderFactory.createTitledBorder(null,
		"Group Information", TitledBorder.TOP, TitledBorder.TOP,
		new Font("times new roman", Font.PLAIN, 12), Color.BLACK));
	...
	listPanel.setBorder(BorderFactory.createTitledBorder(null,
		"Added characters", TitledBorder.TOP, TitledBorder.TOP,
		new Font("times new roman", Font.PLAIN, 12), Color.BLACK));
	...
	final JComboBox<String> realms = new JComboBox<String>(
		realmList.getRealm());
	final JPanel bottomPanel = new JPanel(new GridLayout(1, 2));

	// Modify frame settings
	frame.setIconImage(Toolkit.getDefaultToolkit()
		.getImage(MainFrame.class.getResource("wow.png"))
	...
	rFileButton.addActionListener(new ActionListener() {
		...
		executor.execute(new Runnable() {
			...
			try {
			    tr.search(characters, groupPanel, itemLevel,
				    groupWindow, listPanel, charWindow, frame, prog);
	...
	addCharacterButton.addActionListener(new ActionListener() {
		...
		executor.execute(new Runnable() {
			...
			final CharacterPanel characterPanel = new CharacterPanel(
				textFieldCharacter.getText().toLowerCase());
			JSONObject obj = characterPanel.getURL(
				textFieldCharacter.getText(), realms
					.getSelectedItem().toString());

			// If there are no duplicates and both character name
			// and realm
			// are valid, proceed to button creation
			if (!textFieldCharacter.getText().equals("")
				&& obj != null
				&& !characters.containsKey(textFieldCharacter
					.getText().toLowerCase())) {

			    // Add the character name to an arraylist to keep
			    // track of
			    // duplicates
			    String name = textFieldCharacter.getText()
				    .toLowerCase();
			    ...
			    final JPanel profileButton = new JPanel(
				    new BorderLayout());

			    // Add a remove button inside a panel
			    final JPanel buttonCross = new JPanel(
				    new FlowLayout());
			    ...
			    itemLevel.put(nameDummy,
				    characterPanel.itemLevelValue()); // Add
								      // player
								      // and
								      // the
								      // corresponding
								      ...
								      // a
								      // hashmap
			    groupPanel.avgItemLevelGroup(itemLevel); // Call the
								     // avgItem
			    groupPanel.tierUsers(characters); // ...and the
							      // tierUsers
			    groupWindow.add(groupPanel, BorderLayout.CENTER);

			    // Modify settings for profile button and add the
			    // remove
			    // button to it
			    profileButton.add(nameL, BorderLayout.CENTER);
			    profileButton
				    .setToolTipText("Click to open profile!");
			    ...
			    // Retrieve player and realm values from text
			    // fields.
			    final String player = textFieldCharacter.getText();
			    final String realm = realms.getSelectedItem()
				    .toString();

			    // Go through jsonobject to check which class the
			    // player is
			    // and choose icon and colour based on that.
			    // Using default class colours from:
			    // http://www.wowwiki.com/Class_colors
			    try {
				String playerClass = obj.get("class")
					.toString();
				switch (Integer.parseInt(playerClass)) {
				case 1:
				    JLabel picLabelWarrior = new JLabel(
					    new ImageIcon("images/warrior.png"));
				    profileButton.add(picLabelWarrior,
					    BorderLayout.WEST);
				    float[] colorWarrior = Color.RGBtoHSB(199,
					    156, 110, null);
				    profileButton.setBackground(Color
					    .getHSBColor(colorWarrior[0],
						    colorWarrior[1],
						    colorWarrior[2]));
				    break;
				case 2:
				    JLabel picLabelPaladin = new JLabel(
					    new ImageIcon("images/paladin.png"));
				    profileButton.add(picLabelPaladin,
					    BorderLayout.WEST);
				    float[] colorPaladin = Color.RGBtoHSB(245,
					    140, 186, null);
				    profileButton.setBackground(Color
					    .getHSBColor(colorPaladin[0],
						    colorPaladin[1],
						    colorPaladin[2]));
				    break;
				case 3:
				    JLabel picLabelHunter = new JLabel(
					    new ImageIcon("images/hunter.png"));
				    profileButton.add(picLabelHunter,
					    BorderLayout.WEST);
				    float[] colorHunter = Color.RGBtoHSB(171,
					    212, 115, null);
				    profileButton.setBackground(Color
					    .getHSBColor(colorHunter[0],
						    colorHunter[1],
						    colorHunter[2]));
				    break;
				case 4:
				    JLabel picLabelRogue = new JLabel(
					    new ImageIcon("images/rogue.png"));
				    profileButton.add(picLabelRogue,
					    BorderLayout.WEST);
				    float[] colorRogue = Color.RGBtoHSB(255,
					    245, 105, null);
				    profileButton.setBackground(Color
					    .getHSBColor(colorRogue[0],
						    colorRogue[1],
						    colorRogue[2]));
				    break;
				case 5:
				    JLabel picLabelPriest = new JLabel(
					    new ImageIcon("images/priest.png"));
				    profileButton.add(picLabelPriest,
					    BorderLayout.WEST);
				    float[] colorPriest = Color.RGBtoHSB(255,
					    255, 255, null);
				    profileButton.setBackground(Color
					    .getHSBColor(colorPriest[0],
						    colorPriest[1],
						    colorPriest[2]));
				    buttonCross.setBackground(Color
					    .getHSBColor(colorPriest[0],
						    colorPriest[1],
						    colorPriest[2]));
				    break;
				case 6:
				    JLabel picLabelDeathknight = new JLabel(
					    new ImageIcon(
						    "images/deathknight.png"));
				    profileButton.add(picLabelDeathknight,
					    BorderLayout.WEST);
				    float[] colorDeathKnight = Color.RGBtoHSB(
					    196, 30, 59, null);
				    profileButton.setBackground(Color
					    .getHSBColor(colorDeathKnight[0],
						    colorDeathKnight[1],
						    colorDeathKnight[2]));
				    break;
				case 7:
				    JLabel picLabelShaman = new JLabel(
					    new ImageIcon("images/shaman.png"));
				    profileButton.add(picLabelShaman,
					    BorderLayout.WEST);
				    float[] colorShaman = Color.RGBtoHSB(0,
					    112, 222, null);
				    profileButton.setBackground(Color
					    .getHSBColor(colorShaman[0],
						    colorShaman[1],
						    colorShaman[2]));
				    break;
				case 8:
				    JLabel picLabelMage = new JLabel(
					    new ImageIcon("images/mage.png"));
				    profileButton.add(picLabelMage,
					    BorderLayout.WEST);
				    float[] colorMage = Color.RGBtoHSB(105,
					    204, 240, null);
				    profileButton.setBackground(Color
					    .getHSBColor(colorMage[0],
						    colorMage[1], colorMage[2]));
				    break;
				case 9:
				    JLabel picLabelWarlock = new JLabel(
					    new ImageIcon("images/warlock.png"));
				    profileButton.add(picLabelWarlock,
					    BorderLayout.WEST);
				    float[] colorWarlock = Color.RGBtoHSB(148,
					    130, 201, null);
				    profileButton.setBackground(Color
					    .getHSBColor(colorWarlock[0],
						    colorWarlock[1],
						    colorWarlock[2]));
				    break;
				case 10:
				    JLabel picLabelMonk = new JLabel(
					    new ImageIcon("images/monk.png"));
				    profileButton.add(picLabelMonk,
					    BorderLayout.WEST);
				    float[] colorMonk = Color.RGBtoHSB(0, 255,
					    150, null);
				    profileButton.setBackground(Color
					    .getHSBColor(colorMonk[0],
						    colorMonk[1], colorMonk[2]));
				    break;
				case 11:
				    JLabel picLabelDruid = new JLabel(
					    new ImageIcon("images/druid.png"));
				    profileButton.add(picLabelDruid,
					    BorderLayout.WEST);
				    float[] colorDruid = Color.RGBtoHSB(255,
					    125, 10, null);
				    profileButton.setBackground(Color
					    .getHSBColor(colorDruid[0],
						    colorDruid[1],
						    colorDruid[2]));
				    break;
				}
			    ...
			    final Color defaultColor = profileButton
				    .getBackground();
			    final Color highlightColor = profileButton
				...
				public void actionPerformed(ActionEvent e) {
				    groupPanel.removeAll();
				    characters.remove(nameDummy); // Remove
								  // character
								  // from
								  // duplicate
								  // check
				    itemLevel.remove(nameDummy); // Remove
								 // character
								 // from
								 // average
								 // item
				    ...
				    groupWindow.add(groupPanel,
					    BorderLayout.CENTER);
				    // If charWindow is currently showing info
				    // about the
				    // character about to be deleted, clear the
				    // window
				    if (charWindow.getComponentCount() != 0) {
					if (nameDummy.equals(charWindow
